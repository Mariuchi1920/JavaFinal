...
public class ModificarEquipo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/admin/editarEquipo.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (request.getParameter("editarEntrenador")!=null) {
			/*int idcat= Integer.parseInt(request.getParameter("editarEntrenador"));
			PersonasDAO catpersona = new PersonasDAO();
			catpersona.buscarPersonaId(idcat);*/
			
			request.getSession().setAttribute("editarEntrenador",((Equipo)request.getSession().getAttribute("editador")));
			response.sendRedirect(request.getContextPath() + "/admin/agregarPersonasEquipo");
			
			
		}else if (request.getParameter("elimiarJugador")!=null) {
			int idcat= Integer.parseInt(request.getParameter("elimiarJugador"));
			PersonasDAO catpersona = new PersonasDAO();
			EquiposJugadores equipo = new EquiposJugadores();
			equipo.setEquipo(((Equipo)request.getSession().getAttribute("editador")));
			...
			EquiposJugadores equipo = new EquiposJugadores();
			equipo.setEquipo(((Equipo)request.getSession().getAttribute("editador")));
			equipo.setJugadores(buscadore.listarTodasLosJugadores(equipo.getEquipo()));
			...
			equipo.setCategorias(catCategoria.buscarporIdCategoria(Integer.parseInt(request.getParameter("listaCategorias"))));
			InstitucionesDAO catIntitucion = new InstitucionesDAO();
			equipo.setInstitucion(catIntitucion.buscarPorId(Integer.parseInt(request.getParameter("listaInstitucion"))));
			PersonasDAO catPersona = new PersonasDAO();
			equipo.setEntrenador(catPersona.buscarPersonaId(Integer.parseInt(request.getParameter("listaEntrenadores"))));
